...
public class StopEmpty extends StopBase {
    public static final ResourceLocation RESOURCE_LOCATION = new ResourceLocation(Rustichromia.MODID, "stop_empty");
    public static final ModelResourceLocation ITEM_MODEL = new ModelResourceLocation(new ResourceLocation(Rustichromia.MODID, "cart_control/stop_empty"), "inventory");
    public static final ControlSupplier SUPPLIER = new ControlSupplier(RESOURCE_LOCATION, "stop_empty", ITEM_MODEL) {
        @Override
        public Control get() {
            return new StopEmpty();
        }
    };
    ...
    protected void whileStopped(TileEntityCartControl tile, CartData cart) {
        if(cart.getState().getTime() % 10 == 0) {
            for (EnumFacing facing : EnumFacing.VALUES) {
                cart.content.pushFiltered(facing);
            }
        }
    }
    ...
    public ResourceLocation getTexture(TileEntityCartControl tile, EnumFacing facing) {
        return new ResourceLocation(Rustichromia.MODID,"blocks/cart_control/stop_empty");
    }
}
